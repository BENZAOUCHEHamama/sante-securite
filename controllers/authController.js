...
const signup = async (req, res) => {
  try {
    ...
    /* 🔥 SYNCHRONISATION VERS USERFRAGMENT */
    try {
      await syncUserToFragment(user);
      console.log(`✓ UserFragment créé pour: ${user.email}`);
    } catch (fragmentError) {
      console.error('Erreur lors de la création du UserFragment:', fragmentError);
      // On continue même si la synchronisation échoue pour ne pas bloquer la création du User
    }
...
const login = async (req, res) => {
  try {
    ...
    // Synchroniser vers UserFragment (mise à jour lastLogin)
    try {
      await syncUserToFragment(user);
    } catch (fragmentError) {
...
const generateMfa = async (req, res) => {
  try {
    ...
    // Synchroniser vers UserFragment (mise à jour MFA)
    try {
      await syncUserToFragment(user);
    } catch (fragmentError) {
      console.error('Erreur lors de la synchronisation UserFragment:', fragmentError);
